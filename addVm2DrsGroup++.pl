...
$groupSpec->{'removeKey'}->{'key'} = "1";
$groupSpec->{'info'} = $drsgroup->name;
#$groupSpec->{'info'}->{'vm'} = [ @$groupvms ];
print "VM name: " . $vm_name . "\n";
print "DRS group: " . $drsgroup->name . "\n";
print "Total VM's in group: " . @$groupvms . "\n";
print "Group Spec: " . $groupSpec . "\n";

$clusterSpec = new ClusterConfigSpecEx();
$clusterSpec->{'groupSpec'} = [ $groupSpec ];
print "Cluster Spec: " . $clusterSpec . "\n";
